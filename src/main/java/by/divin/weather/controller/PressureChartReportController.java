...
public class PressureChartReportController {
    @FXML
    ...
    @FXML
    private TextField minPressureYear;

    @FXML
    private TextField maxPressureYear;

    @FXML
    private TextField minPressure;

    @FXML
    private TextField maxPressure;
    ...
    private void setAdditionalInfo(List<WeatherInfo> data) {
        minPressure.setText(String.valueOf(data.get(0).getPressure()));
        maxPressure.setText(String.valueOf(data.get(1).getPressure()));
        minPressureYear.setText(String.valueOf(data.get(0).getYear()));
        maxPressureYear.setText(String.valueOf(data.get(1).getYear()));
    }
    ...
    private Map<Integer, Double> formData(String month) throws UnknownHostException {
        ...
        return reportService.formGraphMonthlyPressureReport(month);
    }

    private List<WeatherInfo> formAdditionalData(String month) throws UnknownHostException {
        IReportService reportService = new IReportServiceImpl();
        return reportService.formExtremumMonthlyPressureInfo(month);
    }
}
